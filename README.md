...
* Ejecutar el siguiente comando
  ...
  ```crearContenedor
    docker build --no-cache=true -t userpermission:latest ./src -f ./ci-cd/docker/Dockerfile --progress=plain
  ```

* Para ejecutar la imagen se debe ejecutar el siguiente comando  
  
  ```ejecutarImagen
    docker run -d -it --rm --name userpermission -p 80:8080 [id-imagen] 
  ```
